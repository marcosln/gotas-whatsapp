...
- THEALOZ: cada 1 hora
- AQUADRAN: cada 8 horas

Al iniciar:
  T+0  min → SYSTANE
  T+10 min → AQUADRAN
  T+30 min → THEALOZ (y desde ahí cada 1h)
"""
...
GIRLFRIEND_NUMBER = '+5219612324432'
YOUR_NUMBER = '+5219612254590'
...
NOTIFICATION_END_HOUR = 2   # Hasta la 1:00 AM del día siguiente
DROP_DELAY_MINUTES = 3       # Separación entre gotas que coinciden exactamente
...
def calculate_drop_schedule(start_time):
    """
    Calcula los horarios de todas las gotas.
    Cada gota inicia en start_time + su start_offset_minutes,
    y luego repite cada interval_hours.
    """
    schedule = {}

    # Hora final: 1 AM del día siguiente si ya pasamos esa hora, o el mismo día
    if NOTIFICATION_END_HOUR <= start_time.hour:
        from datetime import date
        tomorrow = start_time.date() + timedelta(days=1)
        end_of_day = tz.localize(
            datetime.combine(tomorrow, datetime.min.time().replace(hour=NOTIFICATION_END_HOUR))
        )
    else:
        end_of_day = start_time.replace(
            hour=NOTIFICATION_END_HOUR, minute=0, second=0, microsecond=0
        )

    for drop_name, config in DROPS_CONFIG.items():
        interval = config['interval_hours']
        offset = config['start_offset_minutes']
        emoji = config['emoji']

        drop_times = []
        # Primera dosis: start_time + offset
...
def group_drops_by_time(schedule):
    """
    Agrupa las gotas por minuto exacto.
    Si coinciden, las separa por DROP_DELAY_MINUTES.
    """
    time_slots = {}
...
def handle_incoming_message(from_number, message_body):
    ...
    if message in ['INICIAR', 'INICIO', 'HOLA', 'BUENOS DÍAS', 'START', '1']:
        now = datetime.now(tz)
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .drop-card:nth-child(2) { border-left-color: #2196F3; }
            .drop-card:nth-child(3) { border-left-color: #4CAF50; }
            .drop-name { font-weight: bold; color: #333; font-size: 14px; }
            .drop-interval { color: #666; font-size: 12px; margin-top: 4px; }
            .drop-offset { color: #999; font-size: 11px; margin-top: 2px; font-style: italic; }
            .instructions { background: #e8f4fd; border-radius: 12px; padding: 20px; margin-top: 20px; }
    ...
    <body>
        <div class="container">
            ...
            <div class="drop-card">
                ...
                <div class="drop-offset">Primera dosis al iniciar (T+0 min)</div>
            </div>
            <div class="drop-card">
                <div class="drop-name">🌊 AQUADRAN</div>
                <div class="drop-interval">Cada 8 horas</div>
                <div class="drop-offset">Primera dosis a los 10 min de iniciar</div>
            </div>
            <div class="drop-card">
                <div class="drop-name">👁️ THEALOZ</div>
                <div class="drop-interval">Cada 1 hora</div>
                <div class="drop-offset">Primera dosis a los 30 min de iniciar</div>
            </div>
            ...
            <div class="status">✅ Sistema activo y funcionando</div>
        </div>
    </body>
    </html>
    '''
